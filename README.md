...
近期修复：
- v1.2.0.80 优化：
  - 安全：`/api/stats/*` 统一增加登录态鉴权，阻断匿名访问统计数据
  - 安全：CSRF 校验改为基于 `scope.session` 读取，降低中间件顺序变动导致失效的风险
  - 稳定：事件总线改为「有界队列 + 固定 worker」模型，避免高并发事件风暴时线程失控
  - 性能：为播放统计/请求/通知/风控核心表补齐索引，降低大数据量查询开销
  - 体验：用户社区 tab 状态持久化，刷新后保留上次所在页（首页大厅 / 心愿工坊 / 我的画像）
  - 体验：用户社区支持 URL 直达页签（`/request?tab=explore|request|profile`，兼容 hash）
  - 性能：用户社区改为按当前 tab 懒加载数据，减少首屏无效请求
  - 可维护：系统设置改为批量更新后单次落盘，减少重复写配置文件
  - 可维护：移除重复 `BatchActionModel` 定义，避免后续扩展歧义
